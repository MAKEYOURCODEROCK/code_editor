...
def download():
    ...
    try:
        ...
        if os.path.exists('/users/' + username + '/.music/') == True:
            youtube = pytube.YouTube(video_url)
            video = youtube.streams.first()
            filepath = video.download("/users/" + username + "/.music/")
            notif.config(fg="Green", text="Download Complete")
        else:
            os.mkdir('/users/' + username + '/.music/')
            youtube = pytube.YouTube(video_url)
            video = youtube.streams.first()
            video.download("/users/" + username + "/.music/")
            notif.config(fg="Green", text="Download Complete")
